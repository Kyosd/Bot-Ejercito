...
module.exports = {
    ...
    async execute(bot, interaction) {
        ...
        // Asegurar que solo manejamos botones o menús del sistema de sanciones
        if (
        ...
        if (id.startsWith("san_view_")) {
            ...
            return interaction.reply({
                embeds: [embeds.warningsEmbed(soldierTag, warnings)],
                flags: 64
            });
        }
        ...
        if (id.startsWith("do_delete_last_")) {
            ...
            await logSanction(bot, "eliminar_ultima_sancion", interaction.user.id, soldierId, "Última sanción eliminada");
        ...
        if (id.startsWith("do_delete_one_")) {
            ...
            await service.removeOneWarning(bot, soldierId);
            await logSanction(bot, "eliminar_1_advertencia", interaction.user.id, soldierId, "Se eliminó 1 ADV");
        ...
        if (id.startsWith("do_reset_")) {
            ...
            await logSanction(bot, "resetear_advertencias", interaction.user.id, soldierId, "Se reseteó todo el ADV");
        ...
        if (interaction.isStringSelectMenu() && id.startsWith("san_menu_")) {
            ...
            if (value === "list") {
                ...
                return interaction.reply({ embeds: [embeds.listEmbed(list)], flags: 64 });
            }

            if (value === "history") {
                const history = await service.getHistory(bot, soldierId);
                return interaction.reply({ embeds: [embeds.historyEmbed(soldierTag, history)], flags: 64 });
            }
            ...
            if (value === "logs") {
                const rows = await bot.db.all(`SELECT * FROM sanction_logs ORDER BY log_id DESC LIMIT 10`);
...
🎯 Objetivo: ${l.target_soldier_id ? `<@${l.target_soldier_id}>` : "N/A"}  
📝 ${l.detail}  
